...
class AMSTrendScreen extends StatefulWidget {
  AMSTrendScreen(
      {Key? key,
      AreaModel? area_,
  ...
  @override
  State<AMSTrendScreen> createState() => _AMSTrendScreenState();
}

class _AMSTrendScreenState extends State<AMSTrendScreen> {
...
/*  List<DesignPressure> getDesignPress({List<AMSMasterModel>? list}) {
  ...
  List<ActualPressure> getActuallPress({List<AMSMasterModel>? list}) {
    var maxPressure = 100;
  ...
  getDesignPress_new(List<AMSMasterModel>? list) {
    final List<DesignPressure> chartData = [];
    list!.reverse().forEach((element) {
      chartData.add(new DesignPressure(element.amsId!, element.amsNo.toString(),
          element.designPressure ?? 0.0));
  ...
  getActuallPress_new(List<AMSMasterModel>? list) {
    var maxPressure = 100;
    final List<ActualPressure> chartData2 = [];
    list!.reverse().forEach((element) {
      chartData2.add(new ActualPressure(
          ...
          element.actualPressure != null
              ? element.actualPressure * maxPressure / 100
              : 0.0));
    });
    setState(() {
      _Actual = chartData2;
  ...
  Future<List<AMSMasterModel>> getAMSMasterData(
  ...
  getPopupWidget(AMSMasterModel model, {bool? AxisTapped = false}) {
    return Container(
      height: 270,
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Text(
                      model.oNLINE == "ON" ? "ONLINE" : "OFFLINE",
                      textScaleFactor: 1,
                      style: TextStyle(
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    model.lastResponseTime1!.length == 19
                        ? Text(
  ...
  Future<List<AMSMasterModel>>? futureAMSTrendData;
...
/*  AMSDetails(BuildContext context, AMSMasterModel? model) {
    return showDialog(
        barrierDismissible: false,
        useSafeArea: false,
        context: context,
